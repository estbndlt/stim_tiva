...
void USB_write( const uint8_t *pBuffer, size_t nbytes )
{
    int count = 0;
    while (count<nbytes) {
        USBBufferWrite((tUSBBuffer *)&g_sTxBuffer, pBuffer+count, 1);
        count++;
    }
    return;
}
...
void USB_putc( int c )
{
    USBBufferWrite((tUSBBuffer *)&g_sTxBuffer, (uint8_t *)&c, 1);
    return;
}
...
int UART_RxAvailable( void )
{
    ...
    // 0 if receive fifo is empty
    // 1 if receive fifo has something
}
...
USBUARTPrimeTransmit(uint32_t ui32Base)
{
    ...
    while(ROM_UARTSpaceAvail(ui32Base))
    {
        ...
        //ui32Read = USBBufferRead((tUSBBuffer *)&g_sRxBuffer, "L", 1);
        // TODO TEST
               ...
               uint32_t ui32MsgValue, void *pvMsgData)
{
    ...
    case USBD_CDC_EVENT_SET_LINE_CODING:
        //
        // Add this call to keep the same configuration.
        // Bypass the device from getting its settings changed.
        //
        GetLineCoding(pvMsgData);
        //
        // Keep the current serial communication parameters the same.
        //
    ...
    case USBD_CDC_EVENT_SEND_BREAK:
        //
        // This application does not use breaks.
        //
        break;
...
{
    uint32_t ui32Count;
//    uint8_t ui8Char; // TEST
//    uint32_t ui32Read; // TEST
    ...
    {
        //USB_rxAvailable = USBBufferDataAvailable((tUSBBuffer *)&g_sRxBuffer);
        break;
    }
